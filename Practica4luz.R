...
colnames(penguins)
#str me indica el tipo de variable 
str(penguins)
# anyNA datos faltantes
anyNA(penguins)
...
#penguins$isla me permite visualizar 
penguins$isla
...
# ------------------------------------------------
#         Boxplot (GRAFICA DE EXPLORACION)
#--------------------------------------------------

# 1.- Creación de un vector de color
color=c("rosybrown1","thistle1")
...
# 1.- Creación de un vector de color
color=c("palevioletred1", "paleturquoise", "plum2")

# 2.- Creación del gráfico
GB1<-ggplot(penguins, aes(x=año))+
  geom_bar(colour= "midnightblue", fill=color)+
...
HG<-ggplot(penguins, aes(x=largo_aleta_mm))+
  geom_histogram(col="mediumvioletred", fill="mediumpurple1")+
  ggtitle("Histograma")+
...
#----------------------------------
#   Grafico de dispersion ( 2 cuantitativas y 1 cualitativa) 
#-----------------------------------
...
GD<-ggplot(penguins, aes(x=largo_pico_mm, y=grosor_pico_mm))+
  ...
  scale_color_manual(values = c("pink2", "purple2", "tomato1"))+
