...
tabla_PW<-as.data.frame(table(PW = iris$Petal.Width))
...
tabla_clases<-as.data.frame(table (Petal.Width = factor
                                   (cut(iris$Petal.Width,
                                        breaks = 8))))
...
# 7.- Organización visual de la tabla (variable Petal.width)
